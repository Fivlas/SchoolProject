...
<body class="md:flex justify-center bg-black w-full h-full min-h-screen font-['TwitterChirp']">
    ...
    <div class="flex flex-col">
        ...
        <div class="w-full min-h-20 px-0 md:w-[600px] md:min-h-[78px] post">
            ...
            <div class="flex relative justify-between px-[16px] mt-3">
                ...
                <div class="flex flex-col gap-2">
                    <?php
                    if (($_SESSION['isAdmin'] && isset($_GET['admin']) && $_GET['admin'] == "true") || !isset($_GET['u']) || $_GET['u'] == $_SESSION['username']) {
                        echo '<button class="border border-[rgb(83,100,113)] px-4 py-1 rounded-full font-bold transition-all hover:bg-gray-900" onclick="editProfileModal.showModal()">Edytuj profil </button>';
                    } else {
                        echo '<button class="border border-[rgb(83,100,113)] px-4 py-1 rounded-full font-bold transition-all hover:bg-gray-900 invisible" >Edytuj profil </button>';
                    }
                    ?>
                    <a href="../php/actions/logout.php" class="border border-[rgb(83,100,113)] px-4 py-1 rounded-full font-bold transition-all hover:bg-gray-900 md:hidden">Wyloguj się</a>
                </div>
                <dialog id="editProfileModal" class="modal">
                    <div class="modal-box">
                        <form method="POST" action="../php/actions/ChangeUserName.php" enctype="multipart/form-data">
                            ...
                            <div class="flex flex-col justify-center items-center gap-3">
                                ...
                                <input type="text" placeholder=<?php echo $displayName ?> name="username"
                                    class="input input-bordered w-full max-w-xs" />
                                <input type="file" name="avatar" id="avatar" accept="image/*"
                                    class="mt-4 file-input file-input-bordered w-full max-w-xs" />
                                <?php
                                if (isset($_GET['error'])) {
        ...
        if (isset($_GET['u'])) {
            ...
            $queryBuilder->addCondition('user_id', $_SESSION['id']);
        }
                ...
                if (!empty($data)) {
                    ...
                    <div class="flex mt-3 ml-2 md:ml-0">
                        ...
                        <div class="ml-3 flex flex-col">
                            ...
                            <span class="mt-[22px] mb-2 text-blue-400 cursor-pointer"><a href="?tag=' . checkIfTag($tag, true) . '">' . checkIfTag($tag, false) . '</a></span>';
                    if (!empty($postImg)) {
